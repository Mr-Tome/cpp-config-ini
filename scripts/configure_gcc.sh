...
if [[ "$os_type" == "Linux" ]]; then
	if command -v g++ &>/dev/null; then
		echo "g++ $(g++ --version | head -1) already installed."
	else
		echo "g++ not found. Installing via apt..."
		sudo apt-get install -y build-essential
	fi
	# Create marker directory so make_cpp.sh's directory check passes
	mkdir -p "$destination_dep"
else
